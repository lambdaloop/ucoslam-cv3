...
static std::vector<std::string> read(std::string path,std::string ext="",const Params &params=Params()){
    DIR *dir;
    struct dirent *ent;
    auto extensions=parseExtensions(ext);
    ...
    if ((dir = opendir (path.c_str())) != NULL) {
      ...
      while ((ent = readdir (dir)) != NULL){
          ...
          if (addit && !ext.empty()){//check extensionç
              bool foundExt=false;
              for(size_t i=0;i< extensions.size() && !foundExt;i++){
                  std::string thisExt=std::string(fname.begin()+(fname.size()-extensions[i].size()),fname.end());
                  if (thisExt==extensions[i]) foundExt=true;
              }
              if(!foundExt)  addit=false;
          }
...
private:
static std::vector<std::string> parseExtensions(std::string &ext){
    if(ext.empty())return {};
    std::vector<std::string> extensions;
    std::stringstream iss(ext);
    std::string s;
    while ( getline( iss, s, ' ' ) )
